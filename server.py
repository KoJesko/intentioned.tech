...
def _check_ffmpeg():
    ...
    if os.name == "nt":
        ...
        try:
            ...
            print(
                "✅ ffmpeg installed. Please restart the server for PATH changes to take effect."
            )
            sys.exit(0)
...
- Filler Words: {filler.get("count", 0)} used (target: ≤5), rated {filler.get("score", "N/A")}
...
def analyze_delivery_and_tone(
    ...
    delivery_score = (
        ...
        else ("🟡 Good" if len(delivery_issues) <= 1 else "🟠 Needs Work")
    )
    ...
    tone_score = (
        ...
        else ("🟡 Good" if len(tone_issues) <= 1 else "🟠 Needs Work")
    )
...
def check_microaggressions(transcript: list[dict]) -> dict:
    ...
    if detected:
        return {
            "detected": True,
            "feedback": "⚠️ Some phrases may be perceived as microaggressions: "
            + "; ".join(detected),
        }
...
def track_violation_for_repeat_detection(
    ...
    with _violation_tracker_lock:
        ...
        if unique_count == VIOLATION_REPORT_THRESHOLD:
            ...
            print(
                f"⚠️ VIOLATION THRESHOLD REACHED: {unique_count} unique violations from session {session_id[:8]}"
            )
        ...
        if severity == "SEVERE":
            ...
            print(
                f"🚨 SEVERE VIOLATION: {severe_count} severe violation(s) from session {session_id[:8]}"
            )
        ...
        if should_stop or should_stop_severe:
            ...
            print(
                f"🚨 CONVERSATION STOP: {reason} threshold reached for session {session_id[:8]}"
            )
...
async def analyze_session(request: AnalysisRequest):
    ...
    print(
        f"✅ Analysis complete: {filler_analysis['count']} filler words, overall score: {overall}/100"
    )
    ...
    def unload_models(self, force: bool = False):
        ...
        with self._lock:
            ...
            if self._ref_count > 0 and not force:
                print(
                    f"📊 Other sessions active, keeping models (ref_count: {self._ref_count})"
                )
                return
...
async def handle_user_turn(
    ...
    if audio_duration < MIN_AUDIO_DURATION_SEC:
        print(
            f"⚠️ Skipping short audio ({audio_duration:.2f}s < {MIN_AUDIO_DURATION_SEC}s)"
        )
